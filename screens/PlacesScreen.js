...
const PlacesScreen = () => {
  ...
  return (
    <View>
      <Pressable style={styles.filterBar}>
        <Pressable onPress={() => setModalVisible(!modalVisible)} style={styles.Filter}>
      ...
      <BottomModal
        ...
        swipeThreshold={200} footer={<ModalFooter>
          <Pressable onPress={() => applyFilter(selectedFilter)} style={styles.footer}>
      ...
      >
        <ModalContent style={styles.filterSize}>
          <View style={styles.filterTable}>
            ...
            <View style={styles.mapView}>
              {filters.map((item, index) => (
                <Pressable style={styles.mapPress} onPress={() => setSelectedFilter(item.filter)} key={index}>
                  {selectedFilter.includes(item.filter) ? (
                    <FontAwesome name="circle" size={18} color="lightblue" />
                  ) : (
                    <Entypo name="circle" size={18} color="black" />
                  )}
                  <Text style={styles.mapText}>{item.filter}</Text>
                </Pressable>
              ))}
            </View>
          </View>
