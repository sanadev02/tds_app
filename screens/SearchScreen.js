...
const SearchScreen = () => {
    ...
    const handleSearch = async () => {
        setLoading(true);
        try {
            const response = await axios.request(options.listing);
    ...
    // } else {
    //     console.log("Flights data not found in response");
    // }
    // setFlights(response.data.data.flights.map(flight => ({
            ...
            <LinearGradient colors={['#4D5E68', '#96BBBB', 'white',]} style={styles.page}>
                <View >
                    ...
                    <View style={styles.viewBox}>
                        ...
                        {options && options.listings && options.listings.map((listing, index) => (
                            <View key={index} style={{ margin: 10, padding: 10, borderWidth: 1, borderRadius: 10 }}>
                                <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 5 }}>Listing Details</Text>
                                <Text>Location: {listing.location}</Text>
                                <Text>Check-in Date: {listing.checkin_date}</Text>
                                <Text>Check-out Date: {listing.checkout_date}</Text>
                                <Text>Price: £{listing.priceDisplayInfoIrene.displayPrice.amountPerStay.amountRounded}</Text>
                            </View>
                        ))}
